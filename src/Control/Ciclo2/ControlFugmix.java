...
public class ControlFugmix {
    ...
    public ControlFugmix(double T, double P, double zi, Session session){
        ...
        phil_j = (Math.exp((Math.log(vml/(vml-constantesMix.getbm()))))+((constantesMix.getbm()+bpj)/(vml-constantesMix.getbm()))+((apj*D1_l)/(constantes.getR()*T))+(((constantesMix.getam()/(constantes.getR()*T))*F1l_j)-Math.log(zetamix.getZl())))*zi*P;

        //phiv_j=(exp((log(vmv/(vmv-bm)))+((bm+bpj)/(vmv-bm))+((apj*D1_v)/(R*T))+((am/(R*T))*F1v_j)-log(Zvm)))*(1-zi)*P;
        phiv_j = (Math.exp((Math.log(vmv/(vmv-constantesMix.getbm()))))+((constantesMix.getbm()+bpj)/(vmv-constantesMix.getbm()))+((apj*D1_v)/(constantes.getR()*T))+(((constantesMix.getam()/(constantes.getR()*T))*F1v_j)-Math.log(zetamix.getZv())))*zi*P;
